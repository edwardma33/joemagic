  ...
  return (
    <header className='fixed inset-x-0 top-0 z-50 border-b border-slate-200 bg-white/95 backdrop-blur'>
      <nav className='mx-auto flex h-16 max-w-6xl items-center justify-between px-5 lg:px-8' aria-label='Main navigation'>
        <Link href='/' className='flex items-baseline gap-2 text-[#283b46]' onClick={close}>
      ...
      {open && <div className='border-t border-slate-100 bg-white px-5 pb-5 pt-3 md:hidden'>
        <div className='mx-auto flex max-w-7xl flex-col gap-1'>
          {links.map(([label, href]) => <Link key={href} href={href} onClick={close} className='rounded-md px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50'>{label}</Link>)}
          <Link href='/#booking' onClick={close} className='mt-2 rounded-md bg-[#39715a] px-5 py-3 text-center text-sm font-semibold text-white'>Book a show</Link>
        </div>
      </div>}
    </header>
  )
